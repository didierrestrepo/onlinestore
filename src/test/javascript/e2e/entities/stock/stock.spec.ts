...
describe('Stock e2e test', () => {
  ...
  it('should load Stocks', async () => {
    ...
    expect(await stockComponentsPage.getTitle()).to.eq('onlinestoreApp.stock.home.title');
  ...
  it('should load create Stock page', async () => {
    ...
    expect(await stockUpdatePage.getPageTitle()).to.eq('onlinestoreApp.stock.home.createOrEditLabel');
  ...
  /* it('should create and save Stocks', async () => {
        ...
        const selectedEstado = stockUpdatePage.getEstadoInput();
        if (await selectedEstado.isSelected()) {
            await stockUpdatePage.getEstadoInput().click();
            expect(await stockUpdatePage.getEstadoInput().isSelected(), 'Expected estado not to be selected').to.be.false;
        } else {
            await stockUpdatePage.getEstadoInput().click();
            expect(await stockUpdatePage.getEstadoInput().isSelected(), 'Expected estado to be selected').to.be.true;
        }
  ...
  /* it('should delete last Stock', async () => {
        ...
        expect(await stockDeleteDialog.getDialogTitle())
            .to.eq('onlinestoreApp.stock.delete.question');
