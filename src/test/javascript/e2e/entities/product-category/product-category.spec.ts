...
describe('ProductCategory e2e test', () => {
  ...
  it('should load ProductCategories', async () => {
    ...
    expect(await productCategoryComponentsPage.getTitle()).to.eq('onlinestoreApp.productCategory.home.title');
    await browser.wait(
  ...
  it('should load create ProductCategory page', async () => {
    ...
    expect(await productCategoryUpdatePage.getPageTitle()).to.eq('onlinestoreApp.productCategory.home.createOrEditLabel');
  ...
  it('should create and save ProductCategories', async () => {
    ...
    await promise.all([productCategoryUpdatePage.setNameInput('name'), productCategoryUpdatePage.setDescripcionInput('descripcion')]);

    expect(await productCategoryUpdatePage.getNameInput()).to.eq('name', 'Expected Name value to be equals to name');
    expect(await productCategoryUpdatePage.getDescripcionInput()).to.eq(
      'descripcion',
  ...
  it('should delete last ProductCategory', async () => {
    ...
    expect(await productCategoryDeleteDialog.getDialogTitle()).to.eq('onlinestoreApp.productCategory.delete.question');
